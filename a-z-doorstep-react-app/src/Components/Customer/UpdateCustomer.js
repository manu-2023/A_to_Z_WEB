...
export default function UpdateCustomer() {
  ...
  const navigate = useNavigate()
    ...
    function getCustomer(){
        axios.get(baseurl + `/GetCustomerByEmail/${customerEmail}`)
            .then((res) => { 
               setName(res.data.name)
               setMobile(res.data.mobile)
               setEmail(res.data.email)
               setAddress(res.data.address)
               setCityid(res.data.cityCustomer.id)
            ...
            function handleUpdate(e){
                ...
                axios.put(baseurl +`/UpdateCustomer/${customerEmail}/${cid}` ,obj )
                .then((res) => {
                    toast.success('Profile Updated Successfully')
                    sessionStorage.clear(customerEmail);
...
/>
        ...
        <label>Enter Password</label>
          ...
          <div className="d-flex gap-2">
            ...
            <Button variant="secondary" as={Link} to="/customer">
              Back
            </Button>
          </div>
        </Form>
